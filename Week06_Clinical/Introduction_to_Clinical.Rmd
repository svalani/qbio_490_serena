...
```{r setup}
    ...
    knitr::opts_knit$set(root.dir = normalizePath("/home1/serenavalani/490_cluster/analysis_data")) 
```
...
# 1. How many rows are there? How many columns? 1097 rows 112 columns
# 2. What is the data type of $gender? character
# 3. What is the age in years of the 5th patient (barcode: TCGA-4H-AAAK)? 50 years
# 4. How old was the 1st patient at their last follow up (barcode: TCGA-3C-AAAU)? HINT: you need to look at data from two columns to answer this 65 years old
...
```{r}
male_mask <- male_mask <- clinic$gender == "MALE"
```

3) Create a mask to select for patients under the age of 35 OR over 60 at diagnosis. 
```{r}
under30_over60_mask <- under30_over60_mask <- clinic$age_at_diagnosis < 35 | 
                       clinic$age_at_diagnosis > 60
```
...
```{r}
cleaned_clinic$death_event <- ifelse(
  cleaned_clinic$vital_status == "Dead", TRUE, FALSE
)
```
...
# 2. What does the x axis represent? Time in days since diagnosis

# 3. What age group appears to have the greatest rate of survival? The middle age group

# 4. What age group appears to have the worst rate of survival? The old age group

# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients? approx 30%

# 6. What is the p-value? Is it statistically significant and what does that tell us about the relationship between age and breast cancer survival? p = 0.2. It isn't statistically significant because 0.2 > 0.05. This tells us that we don't have strong evidence that survival differs by age group in this dataset. 
