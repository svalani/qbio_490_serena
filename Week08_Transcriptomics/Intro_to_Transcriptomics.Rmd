...
```{r setup}
knitr::opts_knit$set(root.dir = normalizePath("/home1/svalani/490_cluster/Week08_Transcriptomics/analysis_data"))
```
...
Fill out a description of the rows/columns of our data frames. You can use this as a cheat sheet moving forward.
  ...
  ~ rowRanges@elementMetadata
        - Rows: genes
        - Columns: gene annotation metadata
...
(if getwd() prints the wrong directory, run setwd("PATH TO anaylysis_data"))
```{r setup}
knitr::opts_knit$set(root.dir = normalizePath("/home1/svalani/490_cluster/Week08_Transcriptomics/analysis_data"))n
```
...
top-right genes:
Genes that are significantly higher in tumors compared to normal tissue.
bottom-right genes:
Genes that are higher in tumors but are not statistically significant.
top-left genes:
Genes that are significantly lower in tumors compared to normal tissue.
bottom-left genes:
Genes that are lower in tumors but are not statistically significant.
top-middle genes:
Genes that are statistically significant but do not change much in expression.
bottom-middle genes:
Genes that show little change and are not statistically significant.
